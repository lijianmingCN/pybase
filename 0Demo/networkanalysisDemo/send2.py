# -*- coding: utf-8 -*-
#Winpcap 体式格式
#对比 raw sockets 体式格式编程更错杂,但可以本身机关链路层的包。
class SyslogSenderWinPcap:
    """
    道理：先产生一个ping包,以获取本机、网关的MAC地址,然后经由过程WinPcap发包
    限制：今朝不克不及发送给本机,但可以实现；仅支撑数据链路层为以太网的景象
    """
    ...
    def ChooseDevice(self):
        ...
        while True: 
            ...
            for dev in alldevs:
                ...
                print "％d." % index,
                print "％s ％s" % (dev)
            ed = raw_input("Enter the interface number (1-%d):" % index)
            try:
                index = int(ed)
            except TypeError:
    ...
    def GenPing(self, dst):
        ...
        try:
            ...
            icmp = dpkt.icmp.ICMP(type=8, data=dpkt.icmp.ICMP.Echo(id=random.randint(0, 0xffff),seq=1, data="Hello"))
            #sock.connect((ip, 1))
